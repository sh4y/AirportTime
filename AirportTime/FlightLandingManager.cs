...
public class FlightLandingManager
{
    ...
    public bool ProcessFlight(Flight flight, int currentTick)
    {
        ...
        switch (CurrentLandingMode)
        {
            case LandingMode.Automatic:
                return ProcessAutomaticLanding(flight, currentTick);
            case LandingMode.Manual:
                return ProcessManualLanding(flight, currentTick);
            default:
                return ProcessAutomaticLanding(flight, currentTick);
        }
    }
    ...
    /// </summary>
    private bool ProcessAutomaticLanding(Flight flight, int currentTick)
    {
        ...
        CompleteSuccessfulLanding(flight, availableRunway, currentTick);
        return true;
    }
    ...
    /// </summary>
    private bool ProcessManualLanding(Flight flight, int currentTick)
    {
        ...
        if (int.TryParse(input, out int selection))
        {
            ...
            if (selection == availableRunways.Count + 1)
            {
                ...
                result = ProcessAutomaticLanding(flight, currentTick);
            }
            // Delay option
            ...
            else if (selection >= 1 && selection <= availableRunways.Count)
            {
                ...
                {
                    CompleteSuccessfulLanding(flight, selectedRunway, currentTick);
                    result = true;
                }
            }
            else
        ...
        // Show ASCII animation and auto-continue after 3 seconds
    ...
    /// </summary>
    private void CompleteSuccessfulLanding(Flight flight, Runway runway, int currentTick)
    {
        // Mark the runway as occupied for landing
